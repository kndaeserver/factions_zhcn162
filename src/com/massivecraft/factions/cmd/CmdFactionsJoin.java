...
public class CmdFactionsJoin extends FCommand
{
	...
	public void perform()
	{
		...
		if (!samePlayer)
		{
			uplayer.msg("<i>%s <i>你已经加入了 %s<i> 公会.", usender.describeTo(uplayer, true), faction.getName(uplayer));
		}
		...
		if (MConf.get().logFactionJoin)
		{
			...
			{
				Factions.get().log(Txt.parse("%s 允许玩家 %s 加入公会 %s.", usender.getName(), uplayer.getName(), faction.getName()));
			}
		}
	}
}
